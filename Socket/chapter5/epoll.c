/*
(epoll인스턴스 생성 -> 감시할 소켓(등록/수정/삭제) -> 이벤트발생대기)
*/
#include <stdio.h>
#include <unistd.h>
#include <sys/epoll.h>
...
int main()
{
	...
	event.data.fd = STDIN_FILENO;	// 감시할 FD저장
	// STDIN_FILENO fd를 epfd에 등록하여 EPOLLIN이벤트를 감시한ㄷ.
	epoll_ctl(epfd, EPOLL_CTL_ADD, STDIN_FILENO, &event);

	printf("키보드 입력대기중...\n");
	// epfd안의 fd에 이벤트가 발생하면 그 이벤트 정보를 events배열에 넣는다. 한번에 10개까지
