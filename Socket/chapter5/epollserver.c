...
int main(int argc, char* argv[]) {
	...
	ev.data.fd = server_fd;		// 관찰할 fd로 server_fd 저장
	/* 2. 서버 fd의 읽기 이벤트를 감지하도록 epfd 인스턴스에 추가 */
	epoll_ctl(epfd, EPOLL_CTL_ADD, server_fd, &ev);

	while (1){
	/* 3. 이벤트 발생 대기, 인스턴스에 이벤트가 발생하면 발생정보를 events 배열에 처리*/
		event_cnt = epoll_wait(epfd, events, MAX_EVENTS, -1);	// 이벤트발생수 리턴
		if(event_cnt == -1) {
			perror("epoll_wait() failed");
			break;
		}
		for (int i = 0; i < event_cnt; i++) {			// 발생 이벤트만큼 반복처리
			if (events[i].data.fd == server_fd) {		// 발생한 이벤트가 서버 fd이면
				client_fd = accept(server_fd, (struct sockaddr *)&addr, &addrlen);	// 클라이언트
				ev.events = EPOLLIN;	// 이벤트에 읽기 이벤트 저장
				ev.data.fd = client_fd;	// data.fd에 클라이언트 fd를 저장하여
				epoll_ctl(epfd, EPOLL_CTL_ADD, client_fd, &ev);	// epoll인스턴스에 추가
